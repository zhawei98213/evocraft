...
状态：已更新，用于桌面版技术选型和 AI adapter 设计前的工程路线基线；2026-05-17 改为 Electron-first。
...
当前 MVP 继续保留零依赖静态 Web 作为行为和 UI 基线；下一阶段把工程主干迁到 `React + Vite + TypeScript`，同步抽 provider-agnostic AI adapter 与 mock contract tests，然后用 `Electron` 做第一版可安装桌面壳。Tauri 不再作为第一路线，只保留为未来轻量化备选；后端 Web、账号和云同步继续等跨设备、托管密钥或协作需求明确后再启动。
...
| React/Vite 后接 Electron 桌面壳 | Chromium 渲染一致；Node/JS 生态成熟；图片预览、Canvas、拖拽、文件能力和调试路径更可控 | 运行时更重，安装体积和攻击面更大；必须严格设计 preload/IPC 安全边界 | 采用，作为第一版桌面壳 |
| Tauri 2 桌面壳 | 更轻，权限面更小，适合长期轻量化 | 系统 WebView 差异、Rust/Tauri 工具链和插件生态会增加早期验证不确定性 | 保留为未来备选 |
...
- `Vite` 承接开发服务器、构建和未来 Electron renderer 集成。
...
桌面版是下一阶段主交付形态，用户已明确使用 Electron，因此桌面壳从 Tauri-first 改为 Electron-first：

- 第一版桌面壳采用 Electron。
- 第一轮只接必要桌面能力：文件选择、本地应用数据目录、窗口状态和基础打包验证。
- renderer 不启用 Node integration，启用 context isolation 和 sandbox。
- preload 只暴露最小白名单 API，main process 负责文件系统访问，IPC 必须校验来源与输入。
- 本地数据库、系统截图、自动更新、签名、公证和多平台安装包拆成后续独立任务。
- Tauri 保留为未来轻量化、包体积或权限面优化的备选路线，不进入第一版桌面壳实施。
...
2. 写桌面优先实施计划，拆成 React/Vite/TypeScript 迁移、AI adapter contract tests、Electron shell 三条线。
3. 迁移静态 MVP 到 React/Vite/TypeScript，保持产品范围和视觉基线不变。
4. 抽 AI adapter 设计规格和 mock contract tests。
5. 在 React/Vite 主干稳定后接入 Electron 桌面壳。
