...
当前目标：把上传、选择题目区域、识别复核、保存、错题本列表和详情查看闭环加固为可试用 MVP 基线，并继续保持真实 AI/OCR 接入前的本地 mock 策略。
...
- PRD v1.4 采用本地优先、国内模型优先和 provider-agnostic AI 能力契约；UI 阶段可以先使用 mock 识别结果，并把隐私确认、本地删除/清空和失败恢复作为 MVP 收尾要求。
...
- React/Vite 迁移触发点何时达到：AI adapter、复习/分析页面、共享组件或复杂表单状态是否已经让静态 Web 难以维护？
