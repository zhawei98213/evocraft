...
- 第一平台：先做桌面版；下一阶段以 `React + Vite + TypeScript` 为前端工程主干，并在主干稳定后接入 `Tauri 2` 桌面壳。当前静态 Web 保留为 MVP 行为和视觉基线。
- 未来平台：平板和智能手机。
...
当前目标：MVP 收集闭环已完成；下一步按桌面优先技术选型，把静态 Web 基线迁移到 `React + Vite + TypeScript` 工程主干，抽 provider-agnostic AI adapter 与 mock contract tests，并在主干稳定后接入 `Tauri 2` 桌面壳。
...
- 技术路线决策：桌面版优先；当前静态 Web 只保留为 MVP 行为和视觉基线，下一阶段迁移到 `React + Vite + TypeScript`，同步抽 provider-agnostic AI adapter 与 mock contract tests，再接入 `Tauri 2` 做第一版可安装桌面壳。Electron 仅作为 Tauri 不满足关键桌面能力时的 fallback；后端 Web、账号和云同步继续延后。
...
- 桌面版本地持久化具体使用 Tauri Store、SQLite 还是文件夹索引；需要在实施计划中按数据量、查询方式和图片管理方式确认。
- React/Vite/TypeScript 迁移如何分片，才能保持当前 MVP 行为和视觉基线不变。
- Tauri 桌面壳第一轮只接哪些系统能力：文件选择、应用数据目录、窗口状态、打包验证，还是包含自动更新和本地数据库。
