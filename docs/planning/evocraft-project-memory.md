...
- 通过 `@superpowers`、Superpowers 技能或其他 Codex 技能/工具流程产生的项目相关结果，必须保存到本项目目录内；不能只留在聊天、临时目录、浏览器状态、外部工具或 scratch 位置。
...
8. 使用技能或工具 workflow 产生项目相关产物时，必须把 durable 输出保存到项目目录中的合适位置，并纳入同一次提交。
