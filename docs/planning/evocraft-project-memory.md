...
当前目标：把 PRD v1.2 和 B 方案设计基线落成第一个可用的本地 Web MVP，并用真实浏览器验证上传、选择题目区域、识别复核、保存、错题本列表和详情查看闭环。
...
- PRD v1.2 采用本地优先、国内模型优先和 provider-agnostic AI 能力契约；UI 阶段可以先使用 mock 识别结果。
...
- PRD 编写与评审统一参考 `docs/prd/2026-05-16-prd-writing-standards.md`；后续 PRD 应覆盖问题、目标、范围、非目标、用户故事、流程、页面、功能需求、数据、AI/隐私边界、成功指标、验收标准、风险和交接摘要。
...
- PRD 需求语句应尽量单一、必要、无歧义、可验证、可追踪；P0/P1 需求必须能映射到验收方式。
