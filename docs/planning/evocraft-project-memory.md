...
当前目标：MVP 收集闭环已完成；下一步按桌面优先技术选型，把静态 Web 基线迁移到 `React + Vite + TypeScript` 工程主干，抽 provider-agnostic AI adapter 与 mock contract tests，并在主干稳定后接入 `Electron` 桌面壳。
...
- 桌面版本地持久化具体使用 Electron main process 管理的 JSON store、SQLite 还是文件夹索引；需要在实施计划中按数据量、查询方式和图片管理方式确认。
- React/Vite/TypeScript 迁移如何分片，才能保持当前 MVP 行为和视觉基线不变。
- Electron 桌面壳第一轮只接哪些系统能力：文件选择、应用数据目录、窗口状态、打包验证，还是包含自动更新和本地数据库。
