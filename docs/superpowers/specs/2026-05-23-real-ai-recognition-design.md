...
| 任务 | 输入 | 输出 | 第一版策略 |
| --- | --- | --- | --- |
| `region_detection` | 整张原图 | 候选题目区域 | 桌面真实 AI 调用 Qwen provider 返回候选框；adapter 必须归一化 0-1 比例和 Qwen 常见 0-1000 坐标，合成整题候选，并在解析失败时保留整图候选和手动画框兜底 |
...
- 上传页科目选择必须是可执行状态：显式选择 `语文`、`数学` 或 `英语` 时，后续识别请求必须使用该科目；只有 `自动` 模式才要求 provider 返回 subject。
...
- 自动找题返回非法 JSON、空候选或非 0-1 坐标：adapter 先尝试归一化；无法归一化时进入整图候选和手动画框兜底。
- 图片过大：本地压缩或提示重新选择，不直接上传超限图片。
- OCR 失败：保留区域图，允许用户手动填写。
- 结构化 JSON 无效：保存 OCR 原文和失败 trace。
- provider 返回空标题：使用 `识别草稿` 作为可复核标题兜底，不让复核页出现空标题。
- 低置信度：进入复核，不阻断保存。
