...
- React 应用内品牌位使用 `src/assets/evocraft-logo.png`；App Hub 错题收集卡片保持原来的业务入口图标，不被品牌 logo 覆盖。
...
当前导出图已做透明圆角矩形处理；后续发布前仍建议做一版矢量重绘，并补齐 Windows `.ico`、不同平台安全边距检查和真实 dock/installer 视觉验收。
