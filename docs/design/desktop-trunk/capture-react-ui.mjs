...
if (!existsSync(join(distRoot, "index.html"))) {
  throw new Error("Missing dist/index.html. Run `npm run build` before capturing.");
}
...
try {
  const pageWsUrl = await waitForPageWebSocket("http://127.0.0.1:9238");
  ...
  await clickByText(cdp, "button", "下一步：选择题目区域");
  ...
  await clickByText(cdp, "button", "确认此区域并识别");
  await waitForExpression(cdp, "document.body.dataset.screen === 'review'", "review screen");
  await capture(cdp, "04-recognition-review.png");

  await clickByText(cdp, "button", "保存到错题本");
  ...
  await clickByText(cdp, "button", "错题本");
...
async function clickByText(cdp, selector, text) {
  await evaluate(
    ...
    `(() => {
      ...
      const target = targets.find((item) => item.textContent.trim().includes(${JSON.stringify(text)}));
...
function startStaticServer(rootDir) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      const target = resolve(rootDir, pathname.slice(1) || "index.html");
