...
本轮不改 React UI 代码，只完成 Product Design ideation：在基础真实 AI 流程修复之后，为 EvoCraft 桌面错题收集提出 3 个可选视觉/交互方向。每个方向都必须覆盖真实 AI 状态、外部 AI 授权、选区画布、识别复核、错题本和设置边界。

本轮 ImageGen 已生成 3 张方向预览图，并复制到 `docs/design/product-design-directions/`。本方向说明作为后续选择、原型或 React UI 改造的 durable handoff。
...
推荐先选方向 A 作为整体 UI 骨架，同时吸收方向 B 的复核分栏和方向 C 的资料库行列表达。
...
如果选择方向 A，下一步应写一个小步 React UI 改造计划，优先改上传/选区/复核的布局和状态，不一次性重做全站。
