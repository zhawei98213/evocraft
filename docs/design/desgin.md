...
核心画面采用强分栏布局：左侧窄导航保留应用集合身份，中间主工作区展示错题处理结果，右侧 AI 复核面板显示置信度、去痕状态、待确认项和保存动作。蓝色作为主功能色，青绿色只作为 AI 处理和辅助提示色，少量暖黄色用于学习状态、奖励预留和儿童场景温度。

**Key Characteristics:**

- 桌面端优先的三栏工作台：App Hub rail / 错题工作区 / AI 复核面板。
- 蓝色主色，表达 AI、效率、结构化和可信整理。
...
- **Primary Blue** (`{colors.primary}` / `#2563EB`)：主按钮、当前应用、关键进度节点。代表 AI 整理、结构化复核和保存动作。
- **Primary Deep** (`{colors.primary-deep}` / `#1D4ED8`)：按下态、重要文字链接、深色小面积强调。
- **Primary Soft** (`{colors.primary-soft}` / `#E8F0FF`)：状态标签、选中背景、轻量信息块。
- **Primary Mist** (`{colors.primary-mist}` / `#F3F7FF`)：主工作区浅蓝底、干净题面容器和信息分区。
- **AI Cyan** (`{colors.ai-cyan}` / `#06B6D4`)：处理中状态、扫描线、识别步骤节点。只做辅助色，不作为主 CTA。
- **AI Cyan Soft** (`{colors.ai-cyan-soft}` / `#E6FAFD`)：AI 识别草稿、图形保留提示、轻量引导。
...
- **Canvas** (`{colors.canvas}` / `#F7F9FF`)：应用背景。带轻微蓝色倾向，避免纯白刺眼。
- **Surface** (`{colors.surface}` / `#FFFFFF`)：主卡片、表单、题面、右侧面板。
- **Surface Soft** (`{colors.surface-soft}` / `#EEF4FF`)：分区背景、App Hub rail、工具条。
- **Surface Blue** (`{colors.surface-blue}` / `#F3F8FF`)：AI 识别草稿区域。
- **Surface Clean** (`{colors.surface-clean}` / `#F5F8FF`)：干净题面区域。
- **Hairline** (`{colors.hairline}` / `#DCE6F5`)：默认边框和分割线。
- **Hairline Strong** (`{colors.hairline-strong}` / `#B8C8E6`)：输入框、可编辑题面、当前步骤边框。
...
- 主操作只用 `Primary Blue`，不要让青绿色和黄色同时争夺主按钮。
- 蓝色用于主行动、AI 信息和结构化工作台；青绿色只用于处理中或图形保留等辅助提示；玫红用于痕迹或风险；暖黄色用于成长和保存反馈。
- 页面第一眼必须读成蓝色工作台，而不是绿色或蓝绿色应用。
- 背景必须以浅色为主，不做暗色主题。
...
- Background `{colors.surface-clean}`，border `1px solid {colors.hairline}`，rounded `{rounded.lg}`，padding 20px。
...
- Background `{colors.surface-clean}`，border `1px solid {colors.primary-soft}`，rounded `{rounded.lg}`。
...
**`draft-block`**：AI 识别草稿。
- Background `{colors.surface-blue}`，border `1px solid {colors.ai-cyan-soft}`，rounded `{rounded.lg}`，padding 14px。
- 显示为草稿，不要表现成最终正确答案。
...
- Background `{colors.ai-cyan-soft}`，text `{colors.ai-cyan}`，icon 使用扫描或 sparkle 类线性图标。
...
**`subject-badge`**：科目。
- 数学可用 blue，语文可用 violet，英语可用 cyan。
- 科目颜色只做小面积区分，不改变页面主色。
...
Create a polished desktop app UI mockup for EvoCraft, an AI learning assistant app collection for Shanghai children. Use the selected B style direction: future learning workspace, modern AI productivity tool, structured three-panel layout, blue primary visual system, cyan secondary accents, clear status chips, efficient but warm.
...
- light blue background, white and pale-blue clean-question surfaces, blue primary buttons, cyan AI info blocks, rose review-needed chips, small amber success/reward hint;
